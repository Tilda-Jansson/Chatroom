...
(function () {
  ...
  let playerId; // id for player
  let playerRef; // Firebase references
  let players = {}; // local list of states of where all the players are, used to update DOM nodes
  ...
  function initGame() {
    ...
    const allPlayersRef = firebase.database().ref(`players`); // reference to all players
    ...
    allPlayersRef.on("child_added", (snapshot) => {
      //Fires whenever a new node is added the tree, i.e. new player joins
    ...
    function addMessageToDOM(message, messageKey) {
      ...
      if (!characterElement) {
        // If the character element is not available (e.g., the player has left), skip displaying the message.
        return;
      }
    
      const maxLengthPerLine = 20;
      const formattedText = message.text.replace(new RegExp(`(.{${maxLengthPerLine}})`, 'g'), '$1<br>');


      const speechBubble = document.createElement("div");
      speechBubble.classList.add("SpeechBubble");
      if (senderId === playerId) {
        speechBubble.classList.add("sent");
      } else {
        speechBubble.classList.add("received");
      }
    
      speechBubble.innerHTML = `
        <span class="SpeechBubble_text">${formattedText}</span>
      `;
